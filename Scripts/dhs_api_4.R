...
# FUNCTIONS & TESTING: SINGLE YEAR DISAG PLOTTING -----------------------------------------------------------------------

# wrapper function
dhs_viz_nat_disag <- function(indicator, survey_year, breakdown, value_type, output_type) {
...
# function testing
dhs_viz_nat_disag(indicator = "ML_PMAL_C_RDT", 
                  survey_year = 2022,
                  breakdown = "Region",
                  value_type = "Percent",
...
# FUNCTIONS & TESTING: MULTI-YEAR DISAG PLOTTING -------------------------------------------------------------------
...
test <- grab_trend(indicator = indicator_list,
                   country = c("MZ"),
                   breakdown = "Total")


plot_nat_trend(df = df_test,
               color_hex = "#8494FF",
               type = "Absolute",
               c_interval = TRUE)
...
df <- rdhs::dhs_data(indicatorIds = "CM_ECMT_C_NNR",
                     countryIds = "MZ",
                     surveyYearStart = 1997,
                     breakdown = "all")

df1 <- rdhs::dhs_data(indicatorIds = indicator_list,
                     countryIds = "MZ",
                     surveyYearStart = 1997,
                     breakdown = "all") |>  
  dplyr::filter(IsPreferred == 1) |> # in testing phase
  dplyr::select(CountryName, SurveyType, SurveyYear, Indicator, CharacteristicCategory, CharacteristicLabel, CharacteristicId, Value, CIHigh, CILow) |> 
  dplyr::filter(CharacteristicCategory == "all") |>
  dplyr::mutate(CIHigh = ifelse(CIHigh == "", NA, CIHigh),
                CILow = ifelse(CILow == "", NA, CILow))
